...
const Category = () => {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`https://bookland-api.vercel.app/api/theloai/${id}`, {
        method: "DELETE",
      });
      const data = await response.json();
      if (data.status === 1) {
  ...
  const handleEditCategory = async (e) => {
    ...
    try {
      const response = await fetch(
        `https://bookland-api.vercel.app/api/theloai/${currentCategory._id}`,
        {
          // Sử dụng currentCategory._id
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ten: newCategory.ten,
  ...
  return (
    <>
                  ...
                  {currentCategories.map((category) => (
                    <tr key={category._id}>
                      ...
                      <td>
                        ...
                        <button className={styles.details} onClick={() => handleShowMore(category)}>
                          <FaEye />
                        </button>
                      </td>
                    </tr>
      ...
      {showModal && currentCategory && (
        <div className={styles.modal}>
          <div className={styles.modalContent}>
            ...
            <p>
              Ảnh: <img src={currentCategory.img} alt="" style={{ maxWidth: "100px" }} />
            </p>
          ...
          <div className={styles.modalContent}>
            ...
            <form onSubmit={handleAddCategory}>
              ...
              <select
                id="hienthi"
                name="hienthi"
                value={newCategory.hienthi ? "true" : "false"}
                onChange={handleInputChange}
              >
      ...
      {showModalEdit && (
        <div className={styles.modal}>
          <div className={styles.modalContent}>
            <span className={styles.close} onClick={() => setShowModalEdit(false)}>
              &times;
            </span>
            <h2>Sửa Thể Loại</h2>
